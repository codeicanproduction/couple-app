...
export default function SamakanGamePage() {
  ...
  const myAnswerRef = useRef(myAnswer); myAnswerRef.current = myAnswer
  const partnerAnswerRef = useRef(partnerAnswer); partnerAnswerRef.current = partnerAnswer
  const roundResultsRef = useRef(roundResults); roundResultsRef.current = roundResults
  const totalScoreRef = useRef(totalScore); totalScoreRef.current = totalScore
  ...
  // ─── AUDIO ───
  useEffect(() => {
    if (phase === 'countdown' && !audioRef.current) {
      const audio = new Audio('/audio/samakan-bgm.m4a')
      audio.loop = true
      audio.volume = 0.3
      audio.play().catch(() => {}) // ignore autoplay block
      audioRef.current = audio
    }
    if (phase === 'results' && audioRef.current) {
      audioRef.current.pause()
      audioRef.current = null
    }
    return () => {
      if (phase === 'results' || phase === 'connecting') {
        audioRef.current?.pause()
      }
    }
  }, [phase])
  ...
  useEffect(() => {
    async function init() {
      ...
      if (partnerId) {
        const { data: partnerProfile } = await supabase.from('profiles').select('name, avatar_url').eq('id', partnerId).single()
  ...
  useEffect(() => {
    ...
    if (isMyReady && isPartnerReady && chapter) {
      setPhase('countdown')
      broadcast({ type: 'game_start', rounds: chapter.rounds, startAt: Date.now() + 4000 })
      updateSessionPlaying(sessionId)
    }
  }, [isMyReady, isPartnerReady, role, phase, chapter])

  // ─── SCORE ROUND (host only) ───
  const scoreAndReveal = useCallback((hostAns: unknown, guestAns: unknown) => {
    const ch = chapterRef.current
    if (!ch) return
    const round = currentRoundRef.current
    const config = ch.rounds[round]
    const score = getScoreForRound(config.type, hostAns, guestAns)
    const result: RoundResult = { type: config.type, roundIndex: round, config, hostAnswer: hostAns, guestAnswer: guestAns, score }
  ...
  // ─── HANDLE MY ANSWER ───
  const handleMyAnswer = useCallback((answer: unknown) => {
    setMyAnswer(answer)
    broadcast({ type: 'player_answer', playerId: userId, roundIndex: currentRoundRef.current, answer })

    if (roleRef.current === 'host') {
      const pa = partnerAnswerRef.current
      if (pa !== null) scoreAndReveal(answer, pa)
    }
    ...
    onEvent((payload: BroadcastPayload) => {
      switch (payload.type) {
        ...
        case 'round_start':
          ...
          setMyAnswer(null)
          setPartnerAnswer(null)
          // FIX: always set phase (was guarded before, causing stuck bug)
          setPhase('story_bridge')
          break
        case 'player_answer':
  ...
  const advanceFromStory = useCallback(() => {
    ...
    if (p === 'story_intro' || p === 'story_bridge') setPhase('challenge')
    else if (p === 'story_ending') setPhase('results')
  }, [])
  ...
  const handleClose = useCallback(() => { router.push('/app/games/samakan') }, [router])
  ...
  if (phase === 'lobby') {
    return (
      <SamakanLobby
        ...
        onReady={() => { setIsMyReady(true); broadcast({ type: 'player_ready', playerId: userId }) }}
      />
    )
  }
  ...
  if (phase === 'story_intro') {
    return <StoryCard text={chapter.intro} gradient={chapter.bgClass} onNext={handleStoryNext} />
  }
  ...
  if (phase === 'story_bridge') {
    ...
    return <StoryCard text={bridgeText} gradient={chapter.bgClass} onNext={handleStoryNext} />
  }
  ...
  if (phase === 'story_ending') {
    ...
    return <StoryCard text={endingText} gradient={chapter.bgClass} onNext={handleStoryNext} />
  }
    ...
    return (
      <div className={`fixed inset-0 z-50 ${chapter.bgClass}`}>
        ...
        {config.type === 'pilih_sama' && (
          <RoundPilihSama config={config} onSubmit={handleMyAnswer} disabled={myAnswer !== null} />
        )}
        {config.type === 'slider_sama' && (
          <RoundSliderSama config={config} onSubmit={handleMyAnswer} disabled={myAnswer !== null} />
        )}
        {config.type === 'tebak_angka' && (
          <RoundTebakAngka config={config} onSubmit={handleMyAnswer} disabled={myAnswer !== null} />
        )}
